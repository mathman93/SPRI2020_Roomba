...
TURN_CONSTANT = (DISTANCE_CONSTANT)/(WHEEL_SEPARATION) # radians/count
...
def MyLoop(stdscr):
	...
	angle = math.radians(imu.CalculateHeading())
	# Alternatively, set it equal to zero
	#angle = 0.0
	...
	bumper_byte, l_counts_current, r_counts_current, l_speed, r_speed, light_bumper = Roomba.ReadQuery(7,43,44,42,41,45) # Get initial data
		...
		try:
			...
			if Roomba.Available() > 0:
				...
				if delta_l_count-delta_r_count != 0: # delta_angle != 0
					delta_distance *= (2/(delta_angle))*math.sin((delta_angle)/2)
				# Find new x and y position
				x_pos += delta_distance*math.cos(angle-(0.5*delta_angle))
				y_pos += delta_distance*math.sin(angle-(0.5*delta_angle))
				
				# Display current data to the screen
				stdscr.move(5,0)
				stdscr.clrtoeol() # Clear line
				stdscr.addstr(5,0,"{0:.4f}, {1:0>8b}, {2}, {3}, {4:0>8b}, {5}, {6}, {7:.3f}, {8:.3f}, {9:.2f};".format(current_time, bumper_byte, l_counts, r_counts, light_bumper, l_speed, r_speed, y_pos, x_pos, math.degrees(angle)))
				stdscr.move(6,0)
				stdscr.clrtoeol() # Clear line
